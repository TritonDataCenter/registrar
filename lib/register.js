...
function register(opts, cb) {
    ...
    assert.optionalArrayOfString(opts.registration.lbServices,
        'options.registration.lbServices');
    ...
    var cookie = {
        ...
        ].concat((opts.aliases || []).concat(opts.lbServices || []).map(
            function (svc) {
                return path.join(domainToPath(svc), os.hostname());
            })),
        path: p,
        registration: opts.registration,
        zk: opts.zk
